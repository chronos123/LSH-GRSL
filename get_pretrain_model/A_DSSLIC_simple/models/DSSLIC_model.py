...
class DSSLICModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.compG = networks.define_compG(netG_input_nc,
                                           opt.output_nc,
                                           opt.ncf,
                                           opt.n_downsample_comp,
                                           norm=opt.norm,
        ...
        if self.isTrain:
            ...
            base_path = os.path.join(opt.checkpoints_dir, opt.name)
            util.mkdirs(base_path)
            file_name = os.path.join(base_path, 'network.txt')
            time_now = datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
            args = vars(opt)
            with open("{}".format(file_name), "a+") as opt_file:
                opt_file.write(time_now + '\n')
                opt_file.write('------------ Options -------------\n')
                for k, v in sorted(args.items()):
                    opt_file.write('%s: %s\n' % (str(k), str(v)))
                opt_file.write('-------------- End ----------------\n')
                opt_file.write('-------------- Network ----------------\n')
                opt_file.write('-------------- CompG ----------------\n')
                opt_file.write(self.compG.__repr__())
                opt_file.write('-------------- NetG ----------------\n')
                opt_file.write(self.netG.__repr__())
                opt_file.write('-------------- NetD ----------------\n')
                opt_file.write(self.netD.__repr__())
        ...
        if not self.isTrain or opt.continue_train or opt.load_pretrain:
            ...
            self.load_network(self.compG, 'C', opt.which_epoch,
                              pretrained_path)
            if self.isTrain:
        ...
        if self.isTrain:
            ...
            self.loss_names = [
                'G_GAN', 'G_GAN_Feat', 'G_VGG', 'G_DIS', 'G_SSIM', 'D_real',
                'D_fake'
            ]
    ...
    def encode_input(self,
        ...
        if not self.opt.no_seg:  # seg
            if self.opt.label_nc == 0:
                ...
                if flabel_map is not None:
                    input_flabel = torch.cuda.FloatTensor(
                        torch.Size(oneHot_size)).zero_()
                    input_flabel = input_flabel.scatter_(
                        1,
                        flabel_map.data.long().cuda(), 1.0)
    ...
    def forward(self, flabel, image, ds_image, infer=False):
        # Encode Inputs
        input_flabel, real_image, ds_image = self.encode_input(
            flabel, image, ds_image)
        ...
        if self.opt.comp_type == 'compG':  # use compG
            ...
            upsample = torch.nn.Upsample(scale_factor=self.opt.alpha,
                                         mode='bilinear')
        ...
        pred_fake_pool_f = self.discriminate(input_flabel,
                                             fake_image_f,
                                             use_pool=True)
            ...
            for i in range(self.opt.num_D):
                for j in range(len(pred_fake_f[i]) - 1):
                    loss_G_GAN_Feat += D_weights * feat_weights * \
                        ...
                        # l1 loss used for the generator to learn 

        # VGG feature matching loss
        loss_G_VGG = 0
        if not self.opt.no_vgg_loss:
            loss_G_VGG = (self.criterionVGG(fake_image_f,
                                            real_image)) * self.opt.lambda_feat

        # l1 loss between x and x'
        loss_G_DIS = 0
        criterionDIS = torch.nn.L1Loss()
        loss_G_DIS = criterionDIS(fake_image_f,
                                  real_image) * self.opt.lambda_feat * 2
        ...
        return [[
            loss_G_GAN, loss_G_GAN_Feat, loss_G_VGG, loss_G_DIS, loss_G_SSIM,
            loss_D_real, loss_D_fake
        ], None if not infer else real_image,
                None if not infer else input_flabel,
                None if not infer else fake_image_f,
                None if not infer else res, None if not infer else comp_image,
                None if not infer else up_image]

    def inference(self, real, flabel, ds):
        # Encode Inputs
        with torch.no_grad():
            input_flabel, real_image, ds = self.encode_input(
                flabel_map=Variable(flabel),
                real_image=Variable(real),
                ds_image=Variable(ds),
                infer=True)
            ...
            if self.opt.comp_type == 'compG':  # use compG
                ...
                upsample = torch.nn.Upsample(size=(hw[2], hw[3]),
                                             mode='bilinear')
                if not self.opt.no_seg:
    ...
    def renconstruct(self, comp_image):
        # Encode Inputs
        with torch.no_grad():
            comp_image = Variable(comp_image.data.cuda())
            # print("comp image shape is: {}".format(comp_image.shape))
            start_time = time.time()
            upsample = torch.nn.Upsample(scale_factor=self.opt.alpha,
                                         mode='bilinear')
            up_image = upsample(comp_image)

            # print("up image shape is: {}".format(up_image.shape))
...
class DSSLICModelWithVVC(DSSLICModel):
    ...
    def renconstruct(self, comp_image):
        ...
        return fake_image, comp_image

    def name(self):
        return "DSSLIC_COMP_VVC_QP22-Model"
    ...
    def forward(self, flabel, image, ds_image, infer=False):
        ...
        Image.fromarray(util.tensor2im(
            comp_image.data[0])).save("./train_temp/comp.png")

        img_path = run_train_encoder("./train_temp/comp.png",
                                     "vvc",
                                     22,
                                     comp=True)

        comp_image = Image.open(img_path).convert('RGB')

        params = get_params(self.opt, comp_image.size)
        transform_image = get_transform(self.opt, params)
        image_tensor = transform_image(comp_image)

        image_tensor = image_tensor.unsqueeze(0)

        if torch.cuda.is_available():
            comp_image_1 = Variable(image_tensor.data.cuda())
        else:
            comp_image_1 = Variable(image_tensor.data)

        upsample = torch.nn.Upsample(scale_factor=self.opt.alpha,
                                     mode='bilinear')
        up_image = upsample(comp_image_1)
...
class GDNModel(DSSLICModel):
    ### change all the norm layer to GDN, the performance is not good
    def name(self):
        return "DSSLIC_GDN_Model"
    ...
    def initialize(self, opt):
        ...
        self.compG = networks.define_compG(netG_input_nc,
                                           ...
                                           norm="gdn",
                                           gpu_ids=self.gpu_ids)
        self.netG = networks.define_G_GDN(netG_input_nc,
                                      opt.output_nc,
                                      opt.ngf,
                                      opt.netG,
                                      opt.n_downsample_global,
...
class DSSLICModefiedVGG(BaseModel):
    ### change the weight of vgg loss and not use netD, the performance is not good
    def name(self):
        return 'DSSLICModel_modified_vgg_loss'
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            self.criterionFeat = torch.nn.L1Loss()
            if not opt.no_vgg_loss:
                self.criterionVGG = networks.ModifiedVGGLoss(self.gpu_ids)
    ...
    def discriminate(self, input_label, test_image, use_pool=False):
        raise NotImplementedError("no netD")
...
class DSSLICModifiedComp(BaseModel):
    # remove the downsample operation of the generator_network and remove netD and only use the L1 loss
    def name(self):
        return 'DSSLICModel_modified_comp'
...
class DSSLICModell1LossComp(BaseModel):
    ### change the weight of vgg loss and not use netD, the performance is not good
    def name(self):
        return 'DSSLICModel_with_only_l1_and_comp_loss'
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            self.comp_loss_fn = torch.nn.MSELoss()
    ...
    def forward(self, image, infer=False):
        ...
        if self.opt.comp_type == 'compG':  # use compG
            ...
            upsample = torch.nn.Upsample(scale_factor=self.opt.alpha,
                                         mode='bilinear')
            up_image = upsample(comp_image.detach())
        
        else:  # use bicubic downsampling (ds)
            up_image = ds_image
        
        loss_comp = self.comp_loss_fn(up_image, real_image)
        ...
        return [[
            loss_G_GAN, loss_G_GAN_Feat, loss_G_VGG, loss_G_DIS, loss_G_SSIM,
            loss_D_real, loss_D_fake, loss_comp
        ], None if not infer else real_image,
                None if not infer else input_flabel,
                None if not infer else fake_image_f,
                None if not infer else res, None if not infer else comp_image,
